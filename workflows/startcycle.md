...
`agents.md` のルーティング規約に従い、CLI 指定のタスクは既定で Codex / Claude Code へ委譲する（自前処理した場合は journal に理由を記録）。

| Step | 担当 | スキル | 完了条件 / コミット |
|:-:|:---|:---|:---|
| 0 | — | `track_progress.md` `manage_quota.md` | ルーティングモードを決定（preflight）→ Git リポジトリか確認（未初期化なら**開始せず** `git init` を促す）→ `RUN_ID` 採番 → `docs/runs/<RUN_ID>/raw/` と journal ヘッダ作成 → 未コミット変更があれば `chore: baseline before <RUN_ID>` |

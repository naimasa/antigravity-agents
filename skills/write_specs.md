...
- **既存システム**: 既存コードがあれば As-Is のアーキテクチャ・依存関係を分析し、To-Be との差分と破壊的変更の有無を明記する。
- **記録**: `track_progress.md` に従う。停止前に `⏸ Awaiting Approval`、承認後に `✅ Done` を journal へ追記する。設計支援に `claude -p` を使った場合は生出力を `raw/` へ保存する。

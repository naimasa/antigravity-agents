...
- **記録**: `track_progress.md` に従う。停止前に `⏸ Awaiting Approval`、承認後に `✅ Done` を journal へ追記する。既存コードベースの構造把握・アーキテクチャ設計は **Claude Code CLI へ委譲する**（`agents.md` のルーティング規約。モデルは **Sonnet** を基本とし、複雑なアーキテクチャ設計・大規模構造見直しには **Opus** を使用する）。生出力を `raw/` へ保存し、`total_cost_usd` を Cost に記録する。Claude Code CLI の実行で利用制限が発生した場合は、`manage_quota.md` に従い利用可能な代替オプションを提示してユーザーに確認する。

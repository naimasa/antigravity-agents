...
- **委譲（既定）**: 監査本体は **Claude Code CLI へ委譲する**（`claude -p --permission-mode bypassPermissions --model sonnet --output-format json --max-budget-usd 2.00 "<prompt>" < /dev/null > docs/runs/$RUN_ID/raw/<NN>-claude-qa-<topic>.json`、モデルは Sonnet を基本とする）。自前監査で済ませてはならない（`local-only` モード時を除く）。`total_cost_usd` を journal の Cost に記録する。
